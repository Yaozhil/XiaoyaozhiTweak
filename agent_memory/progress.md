...
- 插件底部胶囊作为稳定公众号入口：保留绿点/蓝点状态布局；明确已关注时显示绿点“已关注”，其余显示蓝点“去关注”；点击后先复制公众号名称作为兜底，再关闭并移除插件 view，然后用微信原生 `MMWebViewController/WCWebViewController` 加载用户提供的 mp 主页链接。
...
- 已修正底部关注兜底：点击底部胶囊不再调用外部 `weixin://dl/businessWebview`、`weixin://contacts/profile` 或自建 WKWebView；当前先复制公众号名称兜底，再用微信原生 `MMWebViewController/WCWebViewController` 加载用户指定的 `profile_ext` 主页链接，若内部控制器不可用才提示搜索关注。
...
- 用户补充的大佬插件录屏/截图显示 mp 主页可在微信原生 WebView 内打开，UA 含 `MicroMessenger/8.0.74`，页面报“操作频繁，请稍后再试”更像微信服务端频控；当前已恢复内部 WebView 主页入口，但只调用当前类实际响应的初始化 selector，并用异常保护兜住不兼容签名。
...
- 用户提供 `WeChat-2026-06-05-004816.ips`，崩溃为主线程手势触发后的 `doesNotRecognizeSelector`/`SIGABRT`，调用栈经过 `XiaoyaozhiTweak.dylib`。当前底部胶囊不直接调用自动关注私有 selector；内部 WebView 入口已改为 `respondsToSelector` 探测 + `@try/@catch` 的受保护路径，避免无保护调用不存在的私有构造器。
- 已按用户要求将 `control`、`README.md`、`Core/YZConfigManager.m`、`Core/YZPluginLifecycle.m`、`Guard/YZPrivacyGuard.m` 保持/恢复到版本 `1.0.8`。
- 已完整排查旧代码残留：活跃源码中未发现小游戏/骰子/猜丁壳、外部 `weixin://` 跳转、自建 `WKWebView`、旧乐观关注判断函数或旧版本号残留；底部主页入口仅保留受保护的微信原生 WebView 路线。
...
- 用户补充旧方案曾能正常进入公众号主页但不能上下滑动，且大佬插件打开时能看到顶部进度条慢慢加载；当前按该方向恢复微信原生 WebView 路线：优先创建 `MMWebViewController/WCWebViewController` 并通过 `initWithURL:presentModal:extraInfo:` 等 selector 加载 `profile_ext`，在 push 前先禁用并移除 `YZGlassSheetController.view`，避免透明插件层拦截触摸。
- 已设置一次性线程回访：北京时间 `2026-06-05 16:15` 自动复查公众号黑屏修复、Actions 状态和后续反馈。

## 下一步

- 推送后等待 GitHub Actions 构建结果。
- 真机验证首次弹窗倒计时、确认按钮、自动关注请求、失败提示，以及底部胶囊是否留在定制包内打开 `https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=Mzk2NDE2MjU5Ng==&scene=124`；若无法打开，应只复制公众号名称并提示搜索，不再显示“请在微信客户端打开链接”。
- 当前重点验证：底部右侧是否保留绿点/蓝点布局；点击后是否出现微信原生 WebView 顶部进度条并打开公众号主页，页面是否可上下滑动/可操作；不能再黑屏卡死、外跳官方微信或显示“请在微信客户端打开链接”。
- 继续重点验证：进入公众号相关页面再返回后，底栏是否仍可作为入口使用；常用功能入口是否存在且只震动提示。
- 继续重点验证：点击“常用功能”是否只震动并提示“暂未开放”；点击底部胶囊是否不再闪退。
- 继续重点验证：点击底部胶囊后不能再黑屏卡死；若微信内部 URL 路由未接住，仍应已复制公众号名称，可搜索关注。
