...
- 插件底部胶囊作为稳定公众号入口：保留绿点/蓝点状态布局；明确已关注时显示绿点“已关注”，其余显示蓝点“去关注”；点击后先复制公众号名称作为兜底，再关闭并移除插件 view，然后尝试微信内部 URL 分发服务打开用户提供的 mp 主页链接；命不中时只提示搜索关注，不能再进入黑屏页。
...
- 已修正底部关注兜底：点击底部胶囊不再调用外部 `weixin://dl/businessWebview`、`weixin://contacts/profile` 或自建 WKWebView；当前先复制公众号名称兜底，再尝试微信内部 URL 分发服务加载用户指定的 `profile_ext` 主页链接，若内部路由不可用则提示搜索关注。
...
- 当前重点验证：底部右侧是否保留绿点/蓝点布局；点击后是否能通过微信 URL 路由打开公众号主页；若路由命不中，应只显示“已复制公众号名称，请搜索关注”，不能再黑屏卡死、外跳官方微信或显示“请在微信客户端打开链接”。
