...
- 版本号按用户规则从 `1.0.8` 开始递增；当前同步到 `1.1.1`，后续依次 `1.1.2`、`1.1.3`，跳过 `1.1.0`。
...
- 根据用户真机反馈，点击“投喂一下”会黑屏；用户明确不要赞赏弹窗或其它可见效果，已改为仅触发震动反馈。
- 已将插件内 toast 位置优化到下方关注胶囊上方，点击“已关注”时提示更贴近触发位置。

## 下一步

- 在具备 Theos 的环境中构建 deb。
- 真机安装后测试“投喂一下”是否只有震动反馈、无弹窗、无跳转、无 toast、无黑屏。
