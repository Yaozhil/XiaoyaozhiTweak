...
- 用户真机反馈：点击“投喂一下”直接黑屏；原因指向菜单入口 present 微信私有 `ScanQRCodeLogicController` 后未成功跳转。用户明确不要赞赏弹窗或其它可见效果，当前菜单入口仅保留震动反馈。

## 风险与待确认

- 1.2.7 改为 WCEhance 风格后仍需真机验证微信版本兼容性。
- 真实赞赏页链路依赖微信私有扫码类，类名或 selector 随微信版本变化时仍可能失效。
- 菜单入口当前不再直达真实赞赏页，也不展示赞赏弹层；真实赞赏页直达链路仍需另行验证稳定方案。
