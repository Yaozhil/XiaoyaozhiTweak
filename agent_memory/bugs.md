...
- 底部关注失败兜底曾只提示“请手动搜索关注”，原因是 `openBrandProfile` 过度依赖 `CContactMgr`/本地联系人对象；后续直接 open `https://mp.weixin.qq.com/...` 或自建 WKWebView 访问会跳 Safari/触发“请在微信客户端打开链接”，已改为优先通过微信 `businessWebview` scheme 打开用户提供的公众号主页链接。
